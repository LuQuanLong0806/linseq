  ...
  <div class="task-list-page">
    ...
    <el-card shadow="hover" class="filter-card">
      <el-row :gutter="16" align="middle">
        <el-col :span="6">
          <el-input v-model="filters.keyword" placeholder="搜索任务标题/ID..." :prefix-icon="Search" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
        </el-col>
        <el-col :span="4">
    ...
    <el-card v-show="viewMode === 'table'" shadow="hover" class="table-card">
      ...
      >
        <el-table-column prop="sourceId" label="ID" width="70" />
        <el-table-column prop="title" label="任务标题" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">
            <div class="title-cell">
              <el-tag v-if="taskStore.isInTodoList(row.id)" size="small" class="todo-badge" effect="plain" type="warning">AI待办</el-tag>
              <span>{{ row.title }}</span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="deadline" label="截止时间" width="110" sortable>
          <template #default="{ row }">
            <span :class="{ 'overdue-text': isOverdue(row) }">{{ formatDate(row.deadline) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" link size="small" @click.stop="$router.push(`/tasks/${row.id}`)">详情</el-button>
    ...
    <div v-show="viewMode === 'kanban'" class="kanban-view">
      <div v-for="status in kanbanColumns" :key="status.key" class="kanban-column">
        <div class="kanban-header" :style="{ borderTopColor: status.color }">
          <span class="kanban-title">{{ status.label }}</span>
          <el-tag size="small" round>{{ getTasksByStatus(status.key).length }}</el-tag>
        </div>
          ...
          >
            <div class="kanban-card-header">
              <div class="kanban-left">
                ...
                <el-tag v-if="taskStore.isInTodoList(task.id)" size="small" type="warning" effect="plain">AI待办</el-tag>
              </div>
              <span class="kanban-id">#{{ task.sourceId }}</span>
            </div>
            <div class="kanban-card-title">{{ task.title }}</div>
            <div class="kanban-card-footer">
              <span class="kanban-deadline" :class="{ overdue: isOverdue(task) }">
    ...
    <!-- 项目配置对话框 -->
    <el-dialog v-model="projectDialogVisible" title="项目配置" width="520px" @close="resetProjectForm">
      <el-form :model="projectForm" label-width="110px" label-position="right">
        ...
        <el-form-item label="Git 分支">
          <el-input v-model="projectForm.gitBranch" placeholder="例: feature/task-123" clearable />
        </el-form-item>
        <el-form-item label="任务页面地址">
          <el-input v-model="projectForm.taskPageUrl" placeholder="内网任务页面 URL" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { Search, RefreshRight, Download, ArrowDown, List, Grid, Setting } from '@element-plus/icons-vue'
...
const filteredTasks = computed(() => {
  ...
  if (filters.keyword) {
    ...
    list = list.filter(t => t.title.toLowerCase().includes(kw) || t.sourceId.includes(kw))
  }
...
function getStatusType(status: string): 'success' | 'primary' | 'warning' | 'danger' | 'info' {
  const map: Record<string, 'success' | 'primary' | 'warning' | 'danger' | 'info'> = {
    ...
    submitted: 'success', completed: 'success', rejected: 'danger',
  }
...
function getStatusLabel(status: string): string {
  const labels: Record<string, string> = {
    ...
    submitted: '已提测', completed: '已完结', rejected: '已驳回',
  }
